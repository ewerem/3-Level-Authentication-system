 ...
 <div class="container">
	...
	<center style="border-bottom: 2px solid purple;">
	 	<h6 style="color:black;letter-spacing: 2px;padding: 5px;">
	 		<i>select colors to Authenticate......</i>	 	
	 	</h6>
	</center>
	<br>
...
<!--
	...
	creating the color pattern in which checkbox is used to select
...
<!--
	php code that regulate color pattern for authentication
-->
			...
			<?php
				...
				if(isset($_POST['sub'])){
					...
					if($col == ''){
						...
						$ins = "SELECT color FROM student WHERE email = '$em'";
						$query = $con->query($ins);
						if($query->num_rows > 0){
							...
							if($qy != $color){
								...
								echo'<script>alert("Wrong colours selected !!");</script>';
