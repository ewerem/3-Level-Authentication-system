 ...
 <div class="container">
	...
	<center style="border-bottom: 2px solid purple;">
	 	<h6 style="color:black;letter-spacing: 2px;padding: 5px;">
	 		<i>Please choose colours of your taste.....not less than 4 and not more than 6 colours.</i>	 	
	 	</h6>
	</center>
	<br>
	...
	<div class="row">
		...
		<div class="col l6 offset-l3">
			...
			<br/>
				    <center>
				    <button name="sub" class="waves-effect waves-light purple darken-1 btn-large	">Confirm color pattern <i class="fa fa-arrow-right"></i></button>
	 				</center>
			...
			<?php
				...
				if(isset($_POST['sub'])){
					...
					if($count < 4){
						echo'<script>alert("Colour should not be less than 3 colours!!");</script>';
					}else{
						if($count > 6){
							echo'<script>alert("Colour should not be more than 6 colours !!");</script>';
						}else{
							$ins = "UPDATE student SET color = '$qy' WHERE email = '$em'";
							$query = $con->query($ins);
							if($query){
								
								$_SESSION['student'] = $em;

								header('location:color-pass2.php?em='.$em.''); 
